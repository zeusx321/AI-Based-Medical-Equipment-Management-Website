        ...
        {loading ? (
          ...
          alerts.map((alert, idx) => {
            ...
            return (
              <div
                key={alert.id || idx}
                className={`p-4 rounded-[12px] flex gap-4 items-center bg-color-white/5 border border-color-white/5 hover:bg-color-white/[0.08] transition-all duration-300 ${alert.status === 'NEW' ? 'border-color-purple/20 bg-color-purple/5' : ''}`}
              >
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${severity.colorClass}`}>
                  <img src={severity.icon} alt="Alert" className="w-6" />
                </div>
                <div className="flex-1">
                  <div className="flex justify-between items-start mb-0.5">
                    <h3 className="text-white text-[15px] font-bold uppercase tracking-tight">
                      {alert.type?.replace('_', ' ')}
                    </h3>
                    <span className="text-[11px] text-color-white/30 font-medium">
                      {new Date(alert.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                    </span>
                  </div>
                  <p className="text-[13px] text-color-white/50 leading-relaxed">
                    {alert.message}
                  </p>
                </div>
              </div>
